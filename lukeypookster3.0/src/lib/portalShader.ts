...
/** Standard MVP vertex shader - used on the monitor mesh inside the room. */
...
/** Passthrough vertex shader - used on a drei <ScreenQuad> for the fullscreen gate. */
...
/** Coltrane-green Perlin "portal fire" - carried over from the 2.0 room. */
...
/** Factory for the portal ShaderMaterial (fresh uniforms per instance). */
export function makePortalMaterial(vertexShader: string) {
  return new THREE.ShaderMaterial({
    uniforms: {
      uTime: { value: 0 },
      uReveal: { value: 0 }, // starts unlit; ramped to 1 when the room first comes into view
      uColorStart: { value: new THREE.Color('#bdde87') },
      uColorEnd: { value: new THREE.Color('#159971') },
      uBackground: { value: new THREE.Color('#000000') }, // = section/canvas bg (black) → seamless, no load flip
    },
    vertexShader,
    fragmentShader: PORTAL_FRAG,
    side: THREE.DoubleSide, // monitor plane was invisible from the back face
  });
}
