...
- **WIP flag on the hero:** a small mono "● Work in progress" line (pink pulse dot + cyan text) sits **right after the "An eclectic playground..." intro** in `VaporwaveHero.tsx` (`custom={5}` fade; CTA bumped to `custom={6}`). A top-center shell badge was tried first and Luke couldn't see it - put it in the hero copy flow instead. **Future hero styles should include their own WIP line.**
