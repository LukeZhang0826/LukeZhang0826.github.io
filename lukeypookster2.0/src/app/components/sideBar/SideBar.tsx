...
import MovingScene from '../movingScene/MovingScene';
...
const SideBar = (props: Props) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const newSection = Math.floor((scrollY + halfSection) / sectionHeight);
  ...
  useEffect(() => {
    if (beamRef.current) {
      const containerHeight = 0.5 * window.innerHeight; 
      const sectionHeight = containerHeight / 4; 
      const newTop = (sectionHeight * currentSection) + (sectionHeight / 2) - (containerHeight / 2);
      ...
      // Animating the glow div
      gsap.to('.glow', {
        top: `${25 * currentSection}%`,
        boxShadow: `0 0 60px 5px ${glowColors[currentSection]}`,
        duration: 0.5,
        ease: 'power2.inOut'
      });
    }
  ...
  const handleAnchorClick = (sectionIndex: number) => {
    setCurrentSection(sectionIndex); // Add this line
...
useEffect(() => {
    ...
    switch (currentSection) {
      case 0:
        newConfig = { marginLeft: '25%', y: '0%' };
        break;
      case 1:
        newConfig = { marginLeft: '-10%', y: '0%' };
        break;
      case 2:
        newConfig = { marginLeft: '0%', y: '100%' };
        break;
      case 3:
        newConfig = {  marginLeft: '0%', y: '100%' };
        break;
  ...
  return (
    <>
      <div className="container fixed flex flex-col h-screen z-[10] w-[8%]">
        ...
        <div className="relative h-[50%] w-[100%] flex">
          <div className="h-100 w-[3px] mx-auto flex overflow-hidden z-[1]" style={{ background: 'linear-gradient(#5200FF, #0075FF, #24FF00, #DBFF00)' }}>
            <div
              ref={beamRef}
              className="h-100 w-[1px] bg-slate-50 mx-auto beam"
              style={{
                background: 'linear-gradient(#00000000, #00000000, #FFFFFF, #00000000, #00000000)',
                position: 'relative'
              }}>
            </div>
          </div>

          <div className="absolute h-[25%] w-[3px] left-0 right-0 mx-auto z-[0] glow" />
        ...
        >
          <BiLogoLinkedinSquare 
            className="icon w-[30px] h-[30px] m-3 cursor-pointer" 
            style={{ 
              fill: hoveredIcon === 'linkedin' ? glowColors[currentSection] : '#BBBBBB',
            }}
          />
        </div>
      {/* </MagneticGSAP> */}
            ...
            >
              <BiLogoTelegram 
                className="icon w-[30px] h-[30px] m-3 cursor-pointer" 
                style={{ 
                  fill: hoveredIcon === 'telegram' ? glowColors[currentSection] : '#BBBBBB',
                  // filter: hoveredIcon === 'telegram' ? `drop-shadow(0px 0px 10px ${glowColors[currentSection]})` : 'none' 
                }}
              />
            </div>
            <div
            ...
            >
              <BiLogoGithub 
                className="icon w-[30px] h-[30px] m-3 cursor-pointer" 
                style={{ 
                  fill: hoveredIcon === 'github' ? glowColors[currentSection] : '#BBBBBB',
                  // filter: hoveredIcon === 'github' ? `drop-shadow(0px 0px 10px ${glowColors[currentSection]})` : 'none'
                }}
              />
            </div>
        </div>
      </div>
      <div ref={sceneRef} className="scene fixed container flex justify-center items-center h-screen w-[100vw] z-[1] ml-[100%]">
          <MovingScene />
      </div>
    </>
  );
